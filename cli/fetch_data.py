...
from algo_mvp.models import AlpacaConfig  # BaseConfig for type hint
from algo_mvp.models import BaseConfig, TradovateConfig
...
def main():
    ...
    if not isinstance(raw_config, dict) or "provider" not in raw_config:
        print(
            "Error: Configuration must be a dictionary and include a 'provider' field.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        if provider == "alpaca":
            ...
            print(
                f"Error: Unsupported provider '{provider}'. Supported: alpaca, tradovate.",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        print(
            f"Error: Invalid configuration for provider '{provider}':\n{e}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    try:
        ...
        print(
            f"An critical error occurred during data fetching process: {e}",
            file=sys.stderr,
        )
    ...
    if success:
        ...
        print(
            "CLI: Data fetching process failed or encountered an error.",
            file=sys.stderr,
        )
        sys.exit(1)
