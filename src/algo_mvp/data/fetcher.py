...
from algo_mvp.models import AlpacaConfig, TradovateConfig, BaseConfig
...
class DataFetcher:
    def __init__(self, config: BaseConfig, verbose: bool = False):
        ...
        if isinstance(self.config, AlpacaConfig):
            ...
            self.provider_fetcher = AlpacaFetcher(adjust=self.config.adjust, verbose=self.verbose)
            ...
            raise ValueError(f"Unsupported configuration provider: {self.config.provider}")
    ...
    def _get_base_path(self) -> Path:
        ...
        return Path("data") / self.config.provider / self.config.symbol / timeframe_dirname
    ...
    def _get_parquet_metadata(self, df: pd.DataFrame | None = None) -> dict:
        ...
        if isinstance(self.config, AlpacaConfig):
            meta['alpaca_adjust'] = str(self.config.adjust)  # Store bools as strings for pq metadata
    ...
    def _read_parquet_custom_metadata(self, file_path: Path) -> dict | None:
        try:
            ...
            if self.verbose:
                print(f"Could not read Parquet metadata from {file_path}: {e}", file=sys.stderr)
            return None

    def _check_existing_data(self, target_file: Path, requested_start_dt: pendulum.DateTime, requested_end_dt: pendulum.DateTime) -> bool:
        ...
        if not metadata:
            if self.verbose:
                print(f"Existing file {target_file} found, but no valid metadata to check range. Will re-fetch.")
            return False  # Cannot verify range, so re-fetch

        try:
            # Use 'actual_start_date' or fall back to 'config_start_date' from metadata
            meta_start_str = metadata.get('actual_start_date', metadata.get('config_start_date'))
            meta_end_str = metadata.get('actual_end_date', metadata.get('config_end_date'))

            if not meta_start_str or not meta_end_str:
                if self.verbose:
                    print(f"Metadata in {target_file} missing date range. Will re-fetch.")
                return False
            ...
            try:
                ...
                if self.verbose:
                    print(f"Error parsing date from metadata in {target_file}: {parse_err}. Will re-fetch.")
                return False

            # Ensure requested dates are UTC. requested_end_dt should already be end_of('day').
            req_start_utc = requested_start_dt.in_timezone('UTC').start_of('day')  # Compare start of day
            ...
            if covers_start and covers_end:
                if self.verbose:
                    print(f"Existing data at {target_file} fully covers requested range ({req_start_utc.to_date_string()} to {req_end_utc.to_date_string()}). Skipping download.")
                return True
            else:
                if self.verbose:
                    print(f"Existing data at {target_file} does not fully cover requested range.")
                    print(f"  Requested: {req_start_utc.isoformat()} - {req_end_utc.isoformat()}")
                    print(f"  Existing:  {existing_start_dt.isoformat()} - {existing_end_dt.isoformat()}")
                    print(f"  Covers Start: {covers_start}, Covers End: {covers_end}")
                return False
        except Exception as e:
            # Catch other unexpected errors during comparison
            if self.verbose:
                print(f"Error comparing date ranges from metadata in {target_file}: {e}. Will re-fetch.", file=sys.stderr)
            return False
    ...
    def fetch_data(self, force: bool = False) -> bool:
        ...
        if not force:
            ...
            if self._check_existing_data(single_target_file_path, requested_start_dt, requested_end_dt):
                return True  # Data exists and covers range, success.

        if self.verbose:
            print(f"Proceeding to fetch data for {self.config.symbol} ({self.config.timeframe}) from {self.config.start} to {self.config.end}")
        ...
        if df is None:
            print(f"Failed to fetch data for {self.config.symbol} from provider.", file=sys.stderr)
            return False

        if df.empty:
            if self.verbose:
                print(f"No data returned by provider for {self.config.symbol} for the given range/timeframe. Output file will not be created.")
        ...
        try:
            ...
            if len(unique_years) > 1 and estimated_size_bytes > PARTITION_SIZE_THRESHOLD_BYTES:
                if self.verbose:
                    print(f"Data for {self.config.symbol} spans multiple years and is large ({estimated_size_bytes / (1024*1024):.2f}MB est.). Partitioning by year.")
                ...
                for year in unique_years:
                    ...
                    year_schema_meta = {b'algo_mvp_meta': year_meta_json.encode('utf-8')}
                    ...
                    if self.verbose:
                        print(f"Successfully wrote year-partitioned data to {year_file_path}")
            else:
            ...
            print(f"Error writing Parquet file for {self.config.symbol}: {e}", file=sys.stderr)
