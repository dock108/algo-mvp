...
class AlpacaFetcher:
    def __init__(self, api_key: str | None = None, secret_key: str | None = None, adjust: bool = True, verbose: bool = False):
        ...
        if not self.api_key or not self.secret_key:
            raise ValueError("Alpaca API key and secret key must be provided or set as environment variables ALPACA_KEY_ID and ALPACA_SECRET_KEY.")

        # Using data_v2 for historical bars, paper=True is irrelevant for data.
        self.client = REST(self.api_key, self.secret_key, data_feed='sip')  # SIP for free data
        if self.verbose:
    ...
    @retry(
        ...
        retry=retry_if_exception_type((APIError, ConnectionError))  # Retry on APIError and connection issues
    ...
    def fetch(self, symbol: str, timeframe_str: str, start_date_str: str, end_date_str: str) -> pd.DataFrame | None:
        ...
        start_dt = pendulum.parse(start_date_str).replace(tzinfo=None)  # Alpaca API expects naive datetime for start/end
        ...
        try:
            ...
            value = int(timeframe_str[:-3]) if timeframe_str[-3:].lower() in ["min", "day"] else int(timeframe_str[:-1])
            unit_str = timeframe_str[-3:].lower() if timeframe_str[-3:].lower() in ["min", "day"] else timeframe_str[-1:].lower()
            ...
            if unit_str == "min":
                ...
                raise ValueError(f"Unsupported timeframe unit in '{timeframe_str}'. Supported: Min, Day, H")
        ...
        if self.verbose:
            print(f"Alpaca: Fetching {symbol} ({timeframe_str} -> {timeframe}) from {start_dt.to_iso8601_string()} to {end_dt.to_iso8601_string()}, Adjust: {self.adjust}")
        ...
        try:
            ...
            request_params = {
                ...
                "adjustment": 'split' if self.adjust else 'raw'  # 'all' for dividends and splits, 'split' for splits only
            }
            if self.verbose:
            ...
            if bars_df.empty:
                if self.verbose:
                    print(f"Alpaca: No data returned for {symbol} in the given range and timeframe.")
                return pd.DataFrame()  # Return empty DataFrame, not None, for consistency
            ...
            end_dt_utc = pendulum.parse(end_date_str).in_timezone('UTC').end_of('day')  # Ensure end of day for range
            
            bars_df = bars_df[(bars_df.index >= start_dt_utc) & (bars_df.index <= end_dt_utc)]
            ...
            print(f"Response details: {e._response.text if e._response else 'No response details'}", file=sys.stderr)
            return None
            ...
            print(f"An unexpected error occurred while fetching Alpaca data for {symbol}: {e}", file=sys.stderr)
            return None
...
if __name__ == '__main__':
    ...
    if not api_key_id or not api_secret_key:
        print("ALPACA_KEY_ID and ALPACA_SECRET_KEY must be set in environment for testing.")
        sys.exit(1)
    ...
    spy_minute = fetcher.fetch("SPY", "1Min", "2024-01-01T09:30:00", "2024-01-01T09:35:00")
    ...
    non_existent = fetcher.fetch("NONEXISTENTSYMBOL123", "1Day", "2023-01-01", "2023-01-05")
