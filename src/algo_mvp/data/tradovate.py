...
class TradovateFetcher:
    def __init__(self, client_id: str | None = None, access_token: str | None = None, verbose: bool = False):
        ...
        if not self.client_id:
            ...
            print("Warning: TRADOVATE_CLIENT_ID is not set. Proceeding with access token only.")
        
        if not self.access_token:
            raise ValueError("Tradovate access token must be provided or set as TRADOVATE_ACCESS_TOKEN environment variable.")

        self.session = requests.Session()
        self.session.headers.update({"Authorization": f"Bearer {self.access_token}", "Accept": "application/json"})
        
        if self.verbose:
            print(f"TradovateFetcher initialized for client ID (if provided): {self.client_id}")
    ...
    def _resample_ticks_to_ohlcv(self, ticks_df: pd.DataFrame, timeframe_str: str) -> pd.DataFrame:
        ...
        df_ohlcv.rename(columns={'size': 'volume'}, inplace=True)  # Ensure volume column is named 'volume'
        df_ohlcv = df_ohlcv[['open', 'high', 'low', 'close', 'volume']].copy()
        df_ohlcv.dropna(subset=['open'], inplace=True)  # Remove intervals with no trades
        
        return df_ohlcv

    def fetch(self, symbol: str, timeframe_str: str, start_date_str: str, end_date_str: str) -> pd.DataFrame | None:
        ...
        if self.verbose:
            print(f"Tradovate: Fetching ticks for {symbol} from {start_iso} to {end_iso} for {timeframe_str} resampling.")
        ...
        try:
            ...
            raw_ticks = response_json.get('ticks', [])  # Adjust based on actual API response structure
            if not raw_ticks:
                if self.verbose:
                    print(f"Tradovate: No tick data returned for {symbol} in the given range.")
            ...
            if ticks_df.empty:
                if self.verbose:
                    print(f"Tradovate: Tick data was empty after NaNs dropped for {symbol}.")
                return pd.DataFrame()
            ...
            if self.verbose:
                print(f"Tradovate: Fetched and resampled {len(ohlcv_df)} bars for {symbol}.")
            return ohlcv_df

        except requests.exceptions.HTTPError as e:
            print(f"Tradovate API HTTP Error for {symbol}: {e}. Response: {e.response.text if e.response else 'N/A'}", file=sys.stderr)
            return None
        except requests.exceptions.ConnectionError as e:
            print(f"Tradovate Connection Error for {symbol}: {e}", file=sys.stderr)
            return None
        except Exception as e:
            print(f"An unexpected error occurred while fetching Tradovate data for {symbol}: {e}", file=sys.stderr)
...
if __name__ == '__main__':
    ...
    if not access_token:  # Client ID might be optional for some demo setups if token is direct
    ...
    fetcher = TradovateFetcher(client_id=client_id, access_token=access_token, verbose=True)
    ...
    start_fetch_time = pendulum.now('UTC').subtract(days=2).to_datetime_string()  # e.g. "2024-05-06 10:00:00"
    end_fetch_time = pendulum.parse(start_fetch_time).add(minutes=5).to_datetime_string()
    ...
    if trad_data is not None and not trad_data.empty:
        print(f"Fetched {test_symbol} 1-minute data (first 5 rows):\n{trad_data.head()}")
        print(f"Shape: {trad_data.shape}")
    elif trad_data is not None and trad_data.empty:
        print(f"Fetched {test_symbol} data, but it was empty (no trades in period or after resampling).")
    else:
        print(f"Failed to fetch {test_symbol} data.")

    print("\n--- Test Case 2: Invalid symbol ---")
    invalid_data = fetcher.fetch("INVALIDSYMBOLXYZ123", "1Min", start_fetch_time, end_fetch_time)
    if invalid_data is None:
        print("Correctly failed to fetch data for invalid symbol (returned None due to API error).")
    elif invalid_data.empty:
        print("Correctly returned empty dataframe for invalid symbol (no data or caught by API).")
    else:
